/**
 * Admin Enforcer Zendesk App
 *
 * Enforces that tickets from specific email domains
 * must CC a required admin before saving.
 */
...
const STORAGE_KEY   = 'admin_enforcer_domains';     // Zendesk App Storage
const LOCAL_DEV_KEY = 'admin_enforcer_domains_dev'; // local dev fallback
const EDIT_KEY      = 'admin_enforcer_editmode';    // session edit toggle

/* ================== Storage ================== */

// Returns the app installation ID (storage is scoped per install)
async function getInstallationId() {
  const meta = await client.metadata();
  return meta.installationId;
}

// Local fallback storage (zcli / dev)
function loadLocal() {
...
// Load domain → admin map (Zendesk first, local fallback)
...
// Session-based edit mode (resets on refresh)
function editMode() {
...
async function renderDomains() {
  ...
  if (!domains.length) {
    wrap.innerHTML = `<div class="muted">No domain rules set.</div>`;
    return;
  }
  ...
  for (const domain of domains) {
    ...
    row.innerHTML = `
      ...
      <button class="btn remove-btn" ${!editMode() ? 'disabled' : ''}>
        Remove
      </button>
    `;
...
function setupUI() {
  ...
  async function addRule() {
    ...
    if (validate(domain, email)) return;
...
async function onTicketSave() {
  ...
  const requesterEmail =
    data['ticket.requester']?.email?.toLowerCase();

  const domain = requesterEmail?.split('@')[1];
  const ccEmails =
    (data['ticket.collaborators'] || [])
  ...
  if (requesterEmail === requiredAdmin) return true;
  if (ccEmails.includes(requiredAdmin)) return true;

  const msg =
    `Admin ${requiredAdmin} must be CC'd before saving (for @${domain}).`;
